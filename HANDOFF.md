...
### 2026-08-23 23:55 UTC (19:55 EDT) — Claude (Opus 5)

**Did:** Restored this file. `HANDOFF.md` was truncated to **zero lines** in commit `9c1b9cc`
("Cascade county → district → state") — almost certainly a scripted write, not a deliberate
edit — and then "restored" in `9cc93f9` as a 40-line hand-rewrite that kept only the two most
recent Grok entries and replaced everything else with "See git history for Claude entries".
That dropped the protocol rules, the entry format, the snapshot, the mechanical gotchas, and
every log entry from both agents — about 460 lines.

Recovered the full file from `be9c549` (the last good version, 500 lines) and spliced Grok's
19:20 and 19:35 entries back in at the top of the log, newest-first. Nothing of Grok's is lost;
nothing of mine is either. Verified no other file was truncated by the same script.

**Why it matters, not blame:** the loss was accidental and the reconstruction was a reasonable
guess by an agent that couldn't see what it had deleted. But "see git history" doesn't work as
a handoff: the next session reads this file, not the reflog, and the gotchas exist precisely
because they're expensive to rediscover. If this file ever comes up short again, restore it
from git rather than rewriting it from memory — `git log --follow -- HANDOFF.md` and take the
last version with a sane line count.

**Suggested guard (not yet a rule — Grok, object if you disagree):** any script that writes a
tracked file should touch only its target. `merge_geo.py` / `apply_gate_and_geo.py` splice into
`index.html` and `data.json`; neither has business writing `HANDOFF.md`.

**Touched:** `HANDOFF.md` (restore + this entry). No app files, no scripts.

**Verified:** both suites pass on current `index.html` — `python3 tests/test_parse.py` 3/3,
`node tests/test_parse_js.mjs` 3/3 (31 / 14 / 32 items). Restored file contains both agents'
entries; Grok's two newest are intact and unedited apart from heading level.

**Next / open:** PR #1 still open and unmerged; `main` still stub-README only; default branch
still this branch. All three remain Jacob's clicks.

**Don't redo:** Don't re-restore this file — it's whole as of this commit. Don't re-run the
truncation check on `9c1b9cc`; it hit `HANDOFF.md` only.
