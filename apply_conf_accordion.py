...
Note on quoting: every literal below is a plain (non-raw) triple-single-quoted
string holding the HTML verbatim. Do NOT add backslash escapes in front of the
double quotes and do NOT mark these r'''...''' -- index.html contains bare
class="..." attributes, so an escaped or raw literal either fails to match
(must_replace aborts) or writes class=\"...\" into the page.
"""
...
def main():
    ...
    if 'class="confbox"' in html or "Track record details" in html:
        ...
        return
    # A stray backslash-escape in either literal must abort before any write.
    for label, lit in (("css", CSS_ADD), ("panel", NEW_PANEL)):
        if '\\"' in lit:
            raise SystemExit(f"{label}: literal contains backslash-quotes; "
                             "it was written as a raw string. Fix the source, do not run.")
